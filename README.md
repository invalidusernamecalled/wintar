...
>[!WARNING]
do not use Drag 'n' Drop generally and especially with multiple items (multiple items are not supported/processed by the script. the script does not parse file/directory names correctly in GUI because of the particular unique format that windows uses to provide arguments to the script. instead move multiple items to a single folder to process)
