...
set createparam=-v -c
if "%archive-extension%"==".tar.bz2" set createparam=-v -cj
if "%archive-extension%"==".tar.xz" set createparam=-v -cJ
if "%archive-extension%"==".tar.gz" set createparam=-v -cz
if "%archive-extension%"==".tar.lzma" set createparam=-v -c --lzma
...
if %program_error_level%==0  if exist "%fl_nm_only%%RAND%%archive-extension%" for /f "delims=" %%I in ('tar -t -f "%fl_nm_only%%RAND%%archive-extension%" ^| find /c /v ""') do echo:tar: did not return errors.& if %%I==0 echo File is corrupt.&call :seterror 2
if %program_error_level% NEQ 0  if exist "%fl_nm_only%%RAND%%archive-extension%" for /f "delims=" %%I in ('tar -t -f "%fl_nm_only%%RAND%%archive-extension%" ^| find /c /v ""') do echo:tar: returned some errors.& if %%I==0 echo File is corrupt.&call :seterror 2
